...
<Warning>
  Never expose your Secret Key in client-side code, a public repository, or a mobile app bundle. Use it only from your server.
</Warning>
...
<Warning>
  Protect the script that generates your signature so that only code with the right privileges can call it — it has access to your Secret Key.
</Warning>
...
<Note>
  Pass the parameters as a JSON string, not query-string pairs — e.g. `curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($opts));`
</Note>
